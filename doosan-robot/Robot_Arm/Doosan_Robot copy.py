...
def move_HOME(temp_info_dict):
    ...
    movel(temp_info_dict["location_list"][0], vel=temp_info_dict['vel']['linear'], acc=temp_info_dict["acc"]["linear"])
...
def move_place_point_during_picking_vial(temp_location_dict):
    ...
    for i in range(len(temp_location_dict["location_list"][3])):
        movel(temp_location_dict["location_list"][3][i], vel=temp_location_dict['vel']['linear'], acc = temp_location_dict["acc"]['linear'])
...
def move_ending_point(temp_location_dict):
    ...
    for i in range(len(temp_location_dict["location_list"][6])):
        movel(temp_location_dict["location_list"][6][i], vel=temp_location_dict['vel']['linear'], acc = temp_location_dict["acc"]['linear'])

def pick_and_place(MasterLogger_obj, pick_num, place_num, action_type='storage_empty_to_stirrer', mode_type="virtual"):
    """
        Pick and place function for moving object with DOOSAN robot

        :param MasterLogger_obj (MasterLogger): MasterLogger object to control log message
    ...
    MasterLogger_obj.debug(device_name=device_name, debug_msg=msg)
    ...
    if mode_type=="real":
        movel(temp_info_dict["pick_loc_list"][pick_num], vel=temp_info_dict['vel']['linear'], acc = temp_info_dict["acc"]['linear'])
    ...
    if mode_type=="real":
        movel(temp_info_dict["place_loc_list"][place_num], vel=temp_info_dict['vel']['linear'], acc = temp_info_dict["acc"]['linear'])
...
def move_Storage_empty_to_Stirrer(MasterLogger_obj, cycle_num, place_num, mode_type="virtual"):
    '''
    ...
    pick_and_place(MasterLogger_obj, (cycle_num)//2, place_num, action_type='storage_empty_to_stirrer', mode_type=mode_type)

def move_Stirrer_to_Holder(MasterLogger_obj, place_num, mode_type="virtual"):
    '''
    ...
    :param mode_type (str): "virtual" (str): set mode type --> real, virtual
    '''
    pick_and_place(MasterLogger_obj, place_num, place_num, action_type='stirrer_to_holder', mode_type=mode_type)

def move_Holder_to_Storage_filled(MasterLogger_obj, cycle_num, pick_num, mode_type="virtual"):
    '''
    ...
    :param mode_type (str): "virtual" (str): set mode type --> real, virtual
    '''
    pick_and_place(MasterLogger_obj, pick_num, (cycle_num)//2, action_type='holder_to_storage_filled', mode_type=mode_type)
    
def move_Cuvette_storage_to_Cuvette_holder(MasterLogger_obj, cycle_num, pick_num, mode_type="virtual"):
    """
    ...
    :param mode_type (str): "virtual" (str): set mode type --> real, virtual
    """
    pick_and_place(MasterLogger_obj, (cycle_num)*8+pick_num, pick_num, action_type='cuvette_storage_to_cuvette_holder',mode_type=mode_type)
    
def move_Cuvette_holder_to_UV(MasterLogger_obj, pick_num, mode_type="virtual"):
    """
    ...
    :param mode_type (str): "virtual" (str): set mode type --> real, virtual
    """
    pick_and_place(MasterLogger_obj, pick_num, 0, action_type='cuvette_holder_to_UV',mode_type=mode_type)
   
def move_UV_to_Cuvette_storage(MasterLogger_obj, cycle_num, place_num, mode_type="virtual"):
    """
    ...
    :param mode_type (str): "virtual" (str): set mode type --> real, virtual
    """
    pick_and_place(MasterLogger_obj, 0, (cycle_num)*8+place_num, action_type='UV_to_cuvette_storage',mode_type=mode_type)

def TEST_move_one_cycle(MasterLogger_obj,cycle_num, mode_type="virtual"):
    # tcp_obj=TCP_Class()

    # msg = 'Testing whole action (DOOSAN robot + vial stroage) is started : cycle number is {}.'.format(cycle_num)
    # MasterLogger_obj.info(part_name='SelfdrivingLab', info_msg="Start Robot Queue : "+msg)
    ...
    for action_idx in range(1):
        move_Storage_empty_to_Stirrer(MasterLogger_obj, cycle_num=cycle_num, place_num=action_idx, mode_type=mode_type)
    # for action_idx in range(8):
    #     move_Stirrer_to_Holder(MasterLogger_obj, cycle_num=cycle_num, place_num=action_idx, mode_type=mode_type)


    # for action_idx in range(8):
    #     move_Cuvette_storage_to_Cuvette_holder(MasterLogger_obj, cycle_num=cycle_num, pick_num=action_idx, mode_type=mode_type)
    # for each_vial_loc in range(8):    
    #     move_Cuvette_holder_to_UV(MasterLogger_obj, cycle_num=cycle_num, pick_num=each_vial_loc, mode_type=mode_type)
    #     move_UV_to_Cuvette_storage(MasterLogger_obj, cycle_num=cycle_num, place_num=each_vial_loc, mode_type=mode_type)

    # time.sleep(2)
    # command_bytes=str.encode("{}/{}/{}".format("STORAGE",(cycle_num)//2+6,mode_type))
    # res_msg=tcp_obj.callServer_STORAGE(command_byte=command_bytes)  
    # for action_idx in range(8):
    #     move_Holder_to_Storage_filled(MasterLogger_obj, cycle_num=cycle_num, pick_num=action_idx, mode_type=mode_type)
